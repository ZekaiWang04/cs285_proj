...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            # I assume this is the emsembled clipped double-Q
            assert num_critic_networks >= 2
            idx = torch.randperm(num_critic_networks)[:2]
            next_qs = next_qs[idx, :]
            next_qs = torch.min(next_qs, dim=0)[0]
        else:
            assert self.target_critic_backup_type == "mean"
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qs = self.target_critic(next_obs, next_action) # TODO: get from network
            ...
            target_values: torch.Tensor = reward + torch.where(done, 0, self.discount * next_qs)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        num_simulations = self.num_actor_samples
        samples = action_distribution.rsample((num_simulations,))
        batch_size = samples.shape[1]
        assert samples.shape == (num_simulations, batch_size, self.action_dim), samples.shape
        log_probs = action_distribution.log_prob(samples)
        assert log_probs.shape == (num_simulations, batch_size)
        entropy = -(1/num_simulations) * torch.sum(action_distribution.log_prob(samples), dim=0)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            assert obs.shape == tuple([batch_size] + list(self.observation_shape))
        ...
        assert log_probs.shape == (self.num_actor_samples, batch_size)
        assert advantage.shape == (self.num_actor_samples, batch_size)
        loss = -torch.mean(log_probs * advantage) # remember the negative!
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        assert obs.shape == tuple([batch_size] + list(self.observation_shape))
        assert action.shape == (batch_size, self.action_dim)
        q_values = self.critic(obs, action)
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            info = self.update_critic(observations, 
                                      actions, 
                                      rewards, 
                                      next_observations, 
                                      dones)
        ...
        actor_info = self.update_actor(observations) # TODO: checks
