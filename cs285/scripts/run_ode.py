...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_ob_ensembled = np.array([mb_agent.get_dynamics_predictions(i, ob[np.newaxis, ...], ac[np.newaxis, ...]) for i in range(mb_agent.ensemble_size)])
        next_ob = np.mean(next_ob_ensembled, axis=0)[0]
        ...
        if sac_config is not None:
            ...
            for i in tqdm.trange(
                ...
                sac_agent.update(
                    ...
                    ptu.from_numpy(batch["dones"]).to(bool),
                    i,
                )
